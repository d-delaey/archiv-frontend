<script>
    ...
    async function fetchVods() {
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/vods/?page_size=18&title=${query}`);
        const vods = await response.json();
        return vods
    }

    async function fetchClips() {
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/clips/?page_size=18&title=${query}`);
